...
std::vector<std::pair<double, std::string>> ctc_beam_search_kw_decoder(
    const std::vector<std::vector<double>>& probs_seq,
    const std::vector<std::string>& vocabulary,
    size_t beam_size,
    double cutoff_prob,
    ...
    return get_beam_search_result_kw(prefixes, vocabulary, beam_size, wordlist);
}
...
ctc_beam_search_kw_decoder_batch(
    ...
    std::vector<std::future<std::vector<std::pair<double, std::string>>>> res;
    for (size_t i = 0; i < batch_size; ++i) {
        res.emplace_back(pool.enqueue(ctc_beam_search_kw_decoder,
            probs_split[i],
            vocabulary,
            beam_size,
            cutoff_prob,
